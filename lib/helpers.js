...
export function getHash(preimage, hashFunc = 'sha256') {
  let data = preimage;
  ...
  assert(bcrypto.hasOwnProperty(hashFunc), `Unknown hashing function ${hashFunc}`);
  const hashBuffer = bcrypto[hashFunc].digest(data);
